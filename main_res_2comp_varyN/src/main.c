...
int main() {
	...
	for (u32 j = 0; j < sizeof(g0s)/sizeof(g0s[0]); ++j) {
		...
		for (u32 i = 0; i < sizeof(os)/sizeof(os[0]); ++i) {
			...
			/* RSPT */
			struct pt_result rs_res = rayleigh_schroedinger_pt_rf_2comp(settings, gp_res, g0, occupations);

			/* ENPT */
			struct pt_result en_res = {0};// = en_pt_rf(settings, gp_res, 0, g0, occupations);
